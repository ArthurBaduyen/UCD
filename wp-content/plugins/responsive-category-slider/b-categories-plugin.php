 ...
 * Description: Responsive Category Slider is an amazing combination of slider and content tab. It is the best of both worlds giving your website stunning design, organized contents and easy navigation. Super easy it is to start with listing categories, tags of posts and custom post types. The plugin is empowered by AJAX, CSS3, Bootstrap 3 and WooCommerce compatible.
 * Version: 1.1.0
 ...
 */
	...
	// Version plugin - used on plugin update
	global $wpdb;
	global $b_category_slider;
	$b_category_slider = '1.0.1'; 
	...
	function b_create_table_save_setting_for_category_slider(){
		...
		add_option('b_category_slider', $b_category_slider);

		// repeated for each new version
			$installed_ver = get_option('b_category_slider');
			if ($installed_ver != $b_category_slider) {
				$sql = "CREATE TABLE " . $table_name . " (
				  id int(11) NOT NULL AUTO_INCREMENT,
				  background VARCHAR(100),
				  background_img TEXT,
				...
				// updating option, rather than adding it
				update_option('b_category_slider', $b_category_slider);
			}
		}
	}
	...
	function b_add_id_to_table_b_responsive_category_slider(){
		global $wpdb;
		$table_name = $wpdb->prefix . 'b_responsive_category_slider'; // do not forget about tables prefix
	...
	if ( is_admin() ){
		function b_register_js_file(){
			$siteurl = home_url();
			wp_enqueue_script('jquery', plugin_dir_url( __FILE__ ) . 'js/jquery-1.9.1.min.js', array ( 'jquery' ), '1.0', true);
			wp_enqueue_script('b_category_admin_backend', plugin_dir_url( __FILE__ ) . 'js/b-categories-backend.js', array ( 'jquery' ), '1.0', true);
		}
		add_action('init', 'b_register_js_file');
	}
	else {
		//Register CSS and JS file with font-end
		function b_register_css_and_js() {
			global $wpdb;
			$table_name = $wpdb->prefix . 'b_responsive_category_slider';
			...
			wp_enqueue_script('jquery-1.9.1', plugin_dir_url( __FILE__ ) . 'js/jquery-1.9.1.min.js', 'jquery','1.0',true);
			if(!wp_script_is('flexslider')) {
			wp_enqueue_script('flexslider', plugin_dir_url( __FILE__ ) . 'js/jquery.flexslider.js', 'jquery','1.0',true);
			}
			wp_enqueue_script('jquery-prettyPhoto', plugin_dir_url( __FILE__ ) . 'js/jquery.prettyPhoto.js', 'jquery','1.0',true);
			wp_enqueue_script('jquery-hoverdirection', plugin_dir_url( __FILE__ ) . 'js/jquery-hoverdirection.js', 'jquery','1.0',true);
			wp_enqueue_script('b_category_admin_js_custom', plugin_dir_url( __FILE__ ) . 'js/b-categories.js', 'jquery','1.0',true);
			
		}
		
		add_action('init', 'b_register_css_and_js');
	}
	//Resize thumbnail for slider
	function b_resize_img_thumb(){
	...
	add_action('init', 'b_resize_img_thumb');
	...
	add_shortcode('res-cat-slider', 'create_shortcode_categories_slider');
