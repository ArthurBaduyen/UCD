...
class WDFacebookFeed {
  ...
  protected static $access_tokens = array (
    ...
    '862375440517227|6UamCtWAt-Nn8JEL2vQSM0nKmII',
		'1509500312675877|d4f71636f520624105d7a8e82cc571df', // App version 2.4
		'948188131886213|d7e294429584e7fb5ec8a59a63cad8d8',
		'1697065397194692|65696e3e0679bf806c017cde2c5b4550',
		'1517697358543867|505629efa27c1a99a15f5978e019ef39', //11.03.2015
  );
...
////////////////////////
    ...
    $content = implode(",", self::$content);
      ...
      'app_id' => '',
      'app_secret' => '',
      'exist_access' => self::$exist_access,
  ...
  // Auto update
    ...
    $del_count = 0;
		...
		if((self::$update_mode == 'remove_old') && ($insert_count + $exist_count) > self::$limit) {
			...
			$ids = array();
			$results = $wpdb->get_results($wpdb->prepare('SELECT `id` FROM `wp_wd_fb_data` WHERE  `fb_id` = "%d" ORDER BY `created_time_number` ASC LIMIT ' . $del_count, self::$fb_id));
  ...
  public static function timeline() {
		...
		*/
		...
		$fb_graph_url = str_replace (
      array('{FB_ID}', '{EDGE}','{ACCESS_TOKEN}', '{FIELDS}', '{LIMIT}', '{OTHER}'),
      array(self::$id, $edge, 'access_token='.self::$access_token.'&', $fields, 'limit=100&', ''),
      self::$graph_url
    );
  ...
  public static function specific() {
    ...
    $fb_graph_url = str_replace (
      array('{FB_ID}', '{EDGE}','{ACCESS_TOKEN}', '{FIELDS}', '{LIMIT}', '{OTHER}'),
      array(self::$id, self::$content[0], 'access_token='.self::$access_token.'&', $fields, 'limit='.self::$limit.'&', $other),
      self::$graph_url
    );
